...
// Get tool - make sure it belongs to this vendor
$stmt = $conn->prepare("SELECT * FROM tool_products WHERE id = ? AND vendor_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $imagePath = $tool['image_path']; // keep existing by default
    ...
    if (isset($_FILES['product_image']) && $_FILES['product_image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($ext, $allowedTypes)) {
            ...
            if (move_uploaded_file($_FILES['product_image']['tmp_name'], $destination)) {
                ...
                if ($tool['image_path'] && file_exists(__DIR__ . '/' . $tool['image_path'])) {
                    unlink(__DIR__ . '/' . $tool['image_path']);
                }
    ...
    if (!empty($errors)) {
        ...
        $stmt->bind_param(
            "ssdississsii",
            $productName, $category, $price, $unit, $quantity,
            $brand, $conditionStatus, $description, $status,
            $imagePath, $toolId, $vendorId
        );

        if ($stmt->execute()) {
            $_SESSION['tool_success'] = 'Tool updated successfully!';
            $stmt->close();
            header('Location: vendor-tools.php');
            exit();
        } else {
...
<head>
    ...
    <title>Edit Tool — ChashiBondhu</title>
...
<body class="bg-stone-100 min-h-screen overflow-x-hidden">
    ...
    <main class="max-w-2xl mx-auto px-5 pt-24 pb-16">
        <div class="mb-8">
            <a href="vendor-tools.php" class="inline-flex items-center gap-2 text-amber-700 text-sm font-semibold hover:text-amber-900 transition mb-5">
                <i class="fa-solid fa-arrow-left text-xs"></i> Back to My Tools
            </a>
            <h1 class="text-2xl font-bold text-green-950">Edit Tool</h1>
            <p class="text-stone-400 text-sm mt-1">Update your tool details</p>
        </div>
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-stone-100">
                ...
                <?php if ($tool['image_path']): ?>
                    <div class="mb-4">
                        ...
                        <img src="<?php echo htmlspecialchars($tool['image_path']); ?>" alt="Current" class="w-full h-44 object-cover rounded-xl border border-stone-100">
                    </div>
                <?php endif; ?>

                <label for="product_image" class="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-stone-200 rounded-xl cursor-pointer bg-stone-50 hover:bg-amber-50 hover:border-amber-400 transition relative overflow-hidden">
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-stone-100">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="product_name" required value="<?php echo htmlspecialchars($tool['product_name']); ?>"
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="category" required class="w-full bg-stone-50 border border-stone-200 rounded-xl px-4 py-3 text-sm text-stone-700 outline-none focus:border-amber-500 transition">
                                <?php
                                $cats = ['Tractors & Power Tillers', 'Plows & Harrows', 'Seeders & Planters', 'Irrigation Equipment', 'Harvesters & Threshers', 'Sprayers & Spreaders', 'Hand Tools', 'Seeds', 'Fertilizers & Pesticides', 'Protective Gear', 'Spare Parts', 'Other'];
                                foreach ($cats as $cat): ?>
                                    <option <?php echo $tool['category'] === $cat ? 'selected' : ''; ?>><?php echo $cat; ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div>
                            ...
                            <input type="text" name="brand" value="<?php echo htmlspecialchars($tool['brand'] ?? ''); ?>"
                                class="w-full bg-stone-50 border border-stone-200 rounded-xl px-4 py-3 text-sm text-stone-800 outline-none focus:border-amber-500 transition">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex gap-4">
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="condition_status" value="new" <?php echo $tool['condition_status'] === 'new' ? 'checked' : ''; ?> class="accent-amber-600"> New
                            </label>
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="condition_status" value="used" <?php echo $tool['condition_status'] === 'used' ? 'checked' : ''; ?> class="accent-amber-600"> Used
                            </label>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="description" required rows="4"
                            class="w-full bg-stone-50 border border-stone-200 rounded-xl px-4 py-3 text-sm text-stone-800 outline-none focus:border-amber-500 transition resize-none"><?php echo htmlspecialchars($tool['description']); ?></textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-stone-100">
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="price" required step="0.01" min="0" value="<?php echo $tool['price']; ?>"
                                    class="w-full bg-stone-50 border border-stone-200 rounded-xl pl-8 pr-4 py-3 text-sm text-stone-800 outline-none focus:border-amber-500 transition">
                            </div>
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-stone-500 uppercase tracking-wider mb-2">Unit <span class="text-red-400">*</span></label>
                            <select name="unit" required class="w-full bg-stone-50 border border-stone-200 rounded-xl px-4 py-3 text-sm text-stone-700 outline-none focus:border-amber-500 transition">
                                <?php
                                $units = ['piece', 'set', 'pack', 'kg', 'liter', 'box'];
                                foreach ($units as $u): ?>
                                    <option <?php echo $tool['unit'] === $u ? 'selected' : ''; ?>><?php echo $u; ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="quantity" required min="0" value="<?php echo $tool['quantity']; ?>"
                            class="w-full bg-stone-50 border border-stone-200 rounded-xl px-4 py-3 text-sm text-stone-800 outline-none focus:border-amber-500 transition">
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-stone-500 uppercase tracking-wider mb-2">Status</label>
                        <select name="status" class="w-full bg-stone-50 border border-stone-200 rounded-xl px-4 py-3 text-sm text-stone-700 outline-none focus:border-amber-500 transition">
                            <option value="active" <?php echo $tool['status'] === 'active' ? 'selected' : ''; ?>>● Active</option>
                            <option value="out_of_stock" <?php echo $tool['status'] === 'out_of_stock' ? 'selected' : ''; ?>>● Out of Stock</option>
                            <option value="hidden" <?php echo $tool['status'] === 'hidden' ? 'selected' : ''; ?>>● Hidden</option>
                        </select>
                    </div>
                </div>
            </div>
