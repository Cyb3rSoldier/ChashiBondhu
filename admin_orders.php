...
if ($_GET['delete'] ?? 0) {
    $id = (int)$_GET['delete'];
    $conn->prepare("DELETE FROM orders WHERE id = ?")->bind_param("i", $id)->execute();
    $_SESSION['admin_msg'] = "Order deleted.";
...
$orders = $conn->query("SELECT o.*, c.full_name as consumer FROM orders o LEFT JOIN consumers c ON o.consumer_id = c.id ORDER BY o.created_at DESC")->fetch_all(MYSQLI_ASSOC);
?>
<!DOCTYPE html>
<html><head><title>Product Orders</title><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css"><script src="https://cdn.jsdelivr.net/npm/@tailwindcss/browser@4"></script></head>
<body class="bg-stone-100"><?php include('admin_nav.php'); ?><div class="p-6">
    <h1 class="text-2xl font-bold mb-5">Product Orders</h1>
